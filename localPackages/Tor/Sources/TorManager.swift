...
public final class TorManager: ObservableObject {
    ...
    // `-DSMARTCITYCHAT_DEV_ALLOW_CLEARNET` to temporarily allow direct network.
    public var torEnforced: Bool {
        #if SMARTCITYCHAT_DEV_ALLOW_CLEARNET
        return false
        #else
        return true
        #endif
    ...
    func dataDirectoryURL() -> URL? {
        do {
            ...
            let dir = base.appendingPathComponent("SmartCityChat/tor", isDirectory: true)
            return dir
        } catch {
            return nil
        }
