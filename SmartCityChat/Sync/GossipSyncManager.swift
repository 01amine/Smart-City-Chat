...
final class GossipSyncManager {
    ...
    private struct PacketStore {
        private(set) var packets: [String: SmartCityChatPacket] = [:]
        private(set) var order: [String] = []

        mutating func insert(idHex: String, packet: SmartCityChatPacket, capacity: Int) {
        ...
        func allPackets(isFresh: (SmartCityChatPacket) -> Bool) -> [SmartCityChatPacket] {
        ...
        mutating func remove(where shouldRemove: (SmartCityChatPacket) -> Bool) {
        ...
        mutating func removeExpired(isFresh: (SmartCityChatPacket) -> Bool) {
            remove { !isFresh($0) }
        }
    }
    ...
    private var latestAnnouncementByPeer: [PeerID: (id: String, packet: SmartCityChatPacket)] = [:]
    ...
    func onPublicPacketSeen(_ packet: SmartCityChatPacket) {
    ...
    private func isPacketFresh(_ packet: SmartCityChatPacket) -> Bool {
    ...
    private func isAnnouncementFresh(_ packet: SmartCityChatPacket) -> Bool {
    ...
    private func _onPublicPacketSeen(_ packet: SmartCityChatPacket) {
    ...
    private func buildGcsPayload(for types: SyncTypeFlags) -> Data {
        var candidates: [SmartCityChatPacket] = []
