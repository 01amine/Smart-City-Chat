...
extension ChatViewModel {
    ...
    func subscribeGiftWrap(_ giftWrap: NostrEvent, id: NostrIdentity) {
        ...
        guard let (content, senderPubkey, rumorTs) = try? NostrProtocol.decryptPrivateMessage(giftWrap: giftWrap, recipientIdentity: id),
              content.hasPrefix("SmartCityChat1:"),
              let packetData = Self.base64URLDecode(String(content.dropFirst("SmartCityChat1:".count))),
              let packet = SmartCityChatPacket.from(packetData),
    ...
    func handleGiftWrap(_ giftWrap: NostrEvent, id: NostrIdentity) {
        ...
        guard content.hasPrefix("SmartCityChat1:"),
              let packetData = Self.base64URLDecode(String(content.dropFirst("SmartCityChat1:".count))),
              let packet = SmartCityChatPacket.from(packetData),
              packet.type == MessageType.noiseEncrypted.rawValue,
    ...
    func processNostrMessage(_ giftWrap: NostrEvent) async {
        ...
        do {
            ...
            // Check if it's a SmartCityChat packet embedded in the content (SmartCityChat1:...)
            if content.hasPrefix("SmartCityChat1:") {
                guard let packetData = Self.base64URLDecode(String(content.dropFirst("SmartCityChat1:".count))),
                      let packet = SmartCityChatPacket.from(packetData) else {
                    SecureLogger.error("Failed to decode embedded SmartCityChat packet from Nostr DM", category: .session)
                    return
                }
    ...
    func sendDeliveryAckViaNostrEmbedded(_ message: SmartCityChatMessage, wasReadBefore: Bool, senderPubkey: String, key: Data?) {
