...
/// # ChatViewModel
///
/// The central business logic and state management component for SmartCityChat.
...
/// SwiftUI views and the underlying SmartCityChat services. It manages:
...
/// - **SmartCityChatDelegate**: Receives messages and events from BLEService
...
/// Manages the application state and business logic for SmartCityChat.
/// Acts as the primary coordinator between UI components and backend services,
/// implementing the SmartCityChatDelegate protocol to handle network events.
final class ChatViewModel: ObservableObject, SmartCityChatDelegate, CommandContextProvider, GeohashParticipantContext, MessageFormattingContext {
    ...
    @MainActor
    private func normalizedSenderKey(for message: SmartCityChatMessage) -> String {
    ...
    @Published var messages: [SmartCityChatMessage] = []
    ...
    @Published var allPeers: [SmartCityChatPeer] = []
    var privateChats: [PeerID: [SmartCityChatMessage]] {
    ...
    private var peerIndex: [PeerID: SmartCityChatPeer] = [:]
    ...
    private let nicknameKey = "SmartCityChat.nickname"
    ...
    init(
        ...
        let peersCancellable = unifiedPeerService.$peers
            ...
            .sink { [weak self] peers in
                ...
                var uniquePeers: [PeerID: SmartCityChatPeer] = [:]
                for peer in peers {
    ...
    /// Sends a message through the SmartCityChat network.
    ...
    func sendMessage(_ content: String) {
        ...
        let message = SmartCityChatMessage(
            id: messageID,
    ...
    func blockGeohashUser(pubkeyHexLowercased: String, displayName: String) {
        ...
        if let gh = currentGeohash {
            let predicate: (SmartCityChatMessage) -> Bool = { [self] msg in
    ...
    func removeMessage(withID messageID: String, cleanupFile: Bool = false) {
        var removedMessage: SmartCityChatMessage?
    ...
    func addLocalPrivateSystemMessage(_ content: String, to peerID: PeerID) {
        let systemMessage = SmartCityChatMessage(
            sender: "system",
            content: content,
            timestamp: Date(),
            isRelay: false,
    ...
    @objc private func handleNostrMessage(_ notification: Notification) {
        guard let message = notification.userInfo?["message"] as? SmartCityChatMessage else { return }
    ...
    @objc private func userDidTakeScreenshot() {
        ...
        if let peerID = selectedPrivateChatPeer {
            ...
            let localNotification = SmartCityChatMessage(
                sender: "system",
    ...
    @MainActor
    func getPrivateChatMessages(for peerID: PeerID) -> [SmartCityChatMessage] {
        var combined: [SmartCityChatMessage] = []
        ...
        var bestByID: [String: SmartCityChatMessage] = [:]
        for msg in combined {
    ...
    func panicClearAllData() {
        ...
        userDefaults.removeObject(forKey: "SmartCityChat.noiseIdentityKey")
        userDefaults.removeObject(forKey: "SmartCityChat.messageRetentionKey")
    ...
    @MainActor
    func formatMessageAsText(_ message: SmartCityChatMessage, colorScheme: ColorScheme) -> AttributedString {
        ...
        if message.sender != "system" {
            ...
            senderStyle.font = .SmartCityChatSystem(size: 14, weight: fontWeight, design: .monospaced)
            // Make sender clickable: encode senderPeerID into a custom URL
            if let spid = message.senderPeerID, let url = URL(string: "SmartCityChat://user/\(spid.toPercentEncoded())") {
            ...
            if (content.count > 4000 || content.hasVeryLongToken(threshold: 1024)) && !containsCashuEarly {
                ...
                plainStyle.font = isSelf
                    ? .SmartCityChatSystem(size: 14, weight: .bold, design: .monospaced)
                    : .SmartCityChatSystem(size: 14, design: .monospaced)
            ...
            for (range, type) in allMatches {
                ...
                if let nsRange = Range(range, in: content) {
                    ...
                    if type == "mention" {
                        ...
                        mentionStyle.font = .SmartCityChatSystem(size: 14, weight: isSelf ? .bold : .semibold, design: .monospaced)
                        ...
                        if type == "hashtag" {
                            // If the hashtag is a valid geohash, make it tappable (SmartCityChat://geohash/<gh>)
                            ...
                            if isGeohash && !attachedToMention && standalone, let url = URL(string: "SmartCityChat://geohash/\(token)") {
                                tagStyle.link = url
                            ...
                            matchStyle.font = .SmartCityChatSystem(size: 14, weight: isSelf ? .bold : .semibold, design: .monospaced)
                            if type == "url" {
            ...
            timestampStyle.font = .SmartCityChatSystem(size: 10, design: .monospaced)
            ...
            contentStyle.font = .SmartCityChatSystem(size: 12, design: .monospaced).italic()
    ...
    @MainActor
    func formatMessageHeader(_ message: SmartCityChatMessage, colorScheme: ColorScheme) -> AttributedString {
        let isSelf: Bool = {
            if let spid = message.senderPeerID {
        ...
        if message.sender == "system" {
            ...
            style.font = .SmartCityChatSystem(size: 14, weight: .medium, design: .monospaced)
        ...
        var senderStyle = AttributeContainer()
        senderStyle.foregroundColor = baseColor
        senderStyle.font = .SmartCityChatSystem(size: 14, weight: isSelf ? .bold : .medium, design: .monospaced)
        if let spid = message.senderPeerID,
           let url = URL(string: "SmartCityChat://user/\(spid.id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? spid.id)") {
            senderStyle.link = url
        }
    ...
    @MainActor
    private func peerColor(for message: SmartCityChatMessage, isDark: Bool) -> Color {
        if let spid = message.senderPeerID {
            if spid.isGeoChat || spid.isGeoDM {
    ...
    @MainActor
    func isSelfMessage(_ message: SmartCityChatMessage) -> Bool {
        if let spid = message.senderPeerID {
            // In geohash channels, compare against our per-geohash nostr short ID
            if case .location(let ch) = activeChannel, spid.isGeoChat {
                let myGeo: NostrIdentity? = {
    ...
    @MainActor
    func senderColor(for message: SmartCityChatMessage, isDark: Bool) -> Color {
        return peerColor(for: message, isDark: isDark)
    }

    @MainActor
    func peerURL(for peerID: PeerID) -> URL? {
        return URL(string: "SmartCityChat://user/\(peerID.toPercentEncoded())")
    }
    ...
    private func addMessage(_ message: SmartCityChatMessage) {
    ...
    func getPeer(byID peerID: PeerID) -> SmartCityChatPeer? {
    ...
    // MARK: - SmartCityChatDelegate Methods
    ...
    func didReceiveMessage(_ message: SmartCityChatMessage) {
        Task { @MainActor in
            // Early validation
        ...
        if let current = selectedPrivateChatPeer, current == peerID, let stableKeyHex = derivedStableKeyHex {
            ...
            if let messages = privateChats[peerID] {
                ...
                for msg in messages where !existing.contains(msg.id) {
                    let updated = SmartCityChatMessage(
                        id: msg.id,
                        sender: msg.sender,
                        content: msg.content,
    ...
    func didUpdatePeerList(_ peers: [PeerID]) {
        ...
        DispatchQueue.main.async {
            ...
            // Smart notification logic for "SmartCityChatters nearby"
            ...
            if meshPeerSet.isEmpty {
                ...
                if !newPeers.isEmpty {
                    ...
                    SecureLogger.info(
                        "👥 Sent SmartCityChatters nearby notification for \(meshPeers.count) mesh peers (new: \(newPeers.count))",
                        category: .session
                    )
    ...
    func updateMessageDeliveryStatus(_ messageID: String, status: DeliveryStatus) {
        ...
        for (peerID, chatMessages) in privateChats {
            ...
            // Update delivery status directly (SmartCityChatMessage is a class/reference type)
    ...
    @MainActor
    func handlePublicMessage(_ message: SmartCityChatMessage) {
        ...
        func checkForMentions(_ message: SmartCityChatMessage) {    // Determine our acceptable mention token. If any connected peer shares our nickname,
    ...
    func sendHapticFeedback(for message: SmartCityChatMessage) {        #if os(iOS)
...
extension ChatViewModel: PublicMessagePipelineDelegate {
    func pipelineCurrentMessages(_ pipeline: PublicMessagePipeline) -> [SmartCityChatMessage] {
        messages
    }

    func pipeline(_ pipeline: PublicMessagePipeline, setMessages messages: [SmartCityChatMessage]) {
    ...
    func pipelinePrewarmMessage(_ pipeline: PublicMessagePipeline, message: SmartCityChatMessage) {
