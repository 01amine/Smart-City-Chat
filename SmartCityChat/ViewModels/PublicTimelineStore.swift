...
struct PublicTimelineStore {
    private var meshTimeline: [SmartCityChatMessage] = []
    private var geohashTimelines: [String: [SmartCityChatMessage]] = [:]
    ...
    mutating func append(_ message: SmartCityChatMessage, to channel: ChannelID) {
        switch channel {
        case .mesh:
    ...
    mutating func append(_ message: SmartCityChatMessage, toGeohash geohash: String) {
    ...
    mutating func appendIfAbsent(_ message: SmartCityChatMessage, toGeohash geohash: String) -> Bool {
        var timeline = geohashTimelines[geohash] ?? []
    ...
    mutating func messages(for channel: ChannelID) -> [SmartCityChatMessage] {
        switch channel {
        case .mesh:
            return meshTimeline
        case .location(let channel):
    ...
    @discardableResult
    mutating func removeMessage(withID id: String) -> SmartCityChatMessage? {
    ...
    mutating func removeMessages(in geohash: String, where predicate: (SmartCityChatMessage) -> Bool) {
        var timeline = geohashTimelines[geohash] ?? []
        timeline.removeAll(where: predicate)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }

    mutating func mutateGeohash(_ geohash: String, _ transform: (inout [SmartCityChatMessage]) -> Void) {
        var timeline = geohashTimelines[geohash] ?? []
        transform(&timeline)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }
    ...
    private func trimGeohashTimelineIfNeeded(_ timeline: inout [SmartCityChatMessage]) {
