...
/// - Emits events exclusively via `SmartCityChatDelegate` for UI.
...
final class BLEService: NSObject {
    ...
    private static let centralRestorationID = "chat.SmartCityChat.ble.central"
    private static let peripheralRestorationID = "chat.SmartCityChat.ble.peripheral"
    ...
    private var recentAnnounceBySender: [PeerID: SmartCityChatPacket] = [:]
    ...
    private struct PendingFragmentTransfer {
        let packet: SmartCityChatPacket
        let pad: Bool
        let maxChunk: Int?
    ...
    private var pendingDirectedRelays: [PeerID: [String: (packet: SmartCityChatPacket, enqueuedAt: Date)]] = [:]
    ...
    func sendMessage(_ content: String, mentions: [String] = [], to recipientID: PeerID? = nil, messageID: String? = nil, timestamp: Date? = nil) {
        ...
        let basePacket = SmartCityChatPacket(
    ...
    weak var delegate: SmartCityChatDelegate?
    ...
    func stopServices() {
        ...
        let leavePacket = SmartCityChatPacket(
    ...
    func sendFileBroadcast(_ filePacket: SmartCityChatFilePacket, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
    ...
    func sendFilePrivate(_ filePacket: SmartCityChatFilePacket, to peerID: PeerID, transferId: String) {
        messageQueue.async { [weak self] in
            guard let self = self else { return }
            guard let payload = filePacket.encode() else {
            ...
            var packet = SmartCityChatPacket(
                type: MessageType.fileTransfer.rawValue,
                senderID: self.myPeerIDData,
    ...
    private func broadcastPacket(_ packet: SmartCityChatPacket, transferId: String? = nil) {
        // Apply route if recipient exists (centralized route application)
        let packetToSend: SmartCityChatPacket
    ...
    private func sendEncrypted(_ packet: SmartCityChatPacket, data: Data, pad: Bool) {
    ...
    private func sendGenericBroadcast(_ packet: SmartCityChatPacket, data: Data, pad: Bool) {
    ...
    private func sendOnAllLinks(packet: SmartCityChatPacket, data: Data, pad: Bool, directedOnlyPeer: PeerID?) {
    ...
    private func sendPacketDirected(_ packet: SmartCityChatPacket, to peerID: PeerID) {
    ...
    private func spoolDirectedPacket(_ packet: SmartCityChatPacket, recipientPeerID: PeerID) {
    ...
    private func flushDirectedSpool() {
        ...
        let toSend: [(String, SmartCityChatPacket)] = collectionsQueue.sync(flags: .barrier) {
            var out: [(String, SmartCityChatPacket)] = []
            let now = Date()
    ...
    private func rebroadcastRecentAnnounces() {
        ...
        let packets: [SmartCityChatPacket] = collectionsQueue.sync {
    ...
    private func handleFileTransfer(_ packet: SmartCityChatPacket, from peerID: PeerID) {
        ...
        guard let filePacket = SmartCityChatFilePacket.decode(packet.payload) else {
    ...
    private func handleLeave(_ packet: SmartCityChatPacket, from peerID: PeerID) {
...
extension BLEService: GossipSyncManager.Delegate {
    func sendPacket(_ packet: SmartCityChatPacket) {
        broadcastPacket(packet)
    }

    func sendPacket(to peerID: PeerID, packet: SmartCityChatPacket) {
        sendPacketDirected(packet, to: peerID)
    }

    func signPacketForBroadcast(_ packet: SmartCityChatPacket) -> SmartCityChatPacket {
...
extension BLEService {
    func _test_handlePacket(_ packet: SmartCityChatPacket, fromPeerID: PeerID) {
...
extension BLEService: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        guard let service = services.first(where: { $0.uuid == BLEService.serviceUUID }) else {
            // Not a SmartCityChat peer - disconnect
    ...
    private func processNotificationPacket(_ packet: SmartCityChatPacket, from peripheral: CBPeripheral, peripheralUUID: String) {
    ...
    func peripheral(_ peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
        ...
        if !hasOurService {
            ...
            SecureLogger.warning("❌ SmartCityChat service removed - disconnecting from \(peripheral.name ?? peripheral.identifier.uuidString)", category: .session)
            centralManager?.cancelPeripheralConnection(peripheral)
        } else {
            // Try to rediscover
            peripheral.discoverServices([BLEService.serviceUUID])
...
extension BLEService {
    ...
    private func applyRouteIfAvailable(_ packet: SmartCityChatPacket, to recipient: PeerID) -> SmartCityChatPacket {
        ...
        let routedPacket = SmartCityChatPacket(
            type: packet.type,
    ...
    private func forwardAlongRouteIfNeeded(_ packet: SmartCityChatPacket) -> Bool {
    ...
    private func makeMessageID(for packet: SmartCityChatPacket) -> String {
    ...
    private func priority(for packet: SmartCityChatPacket, data: Data) -> OutboundPriority {
    ...
    private func sendFragmentedPacket(_ packet: SmartCityChatPacket, pad: Bool, maxChunk: Int? = nil, directedOnlyPeer: PeerID? = nil, transferId: String? = nil) {
    ...
    private func startFragmentedPacket(_ context: PendingFragmentTransfer) {
        ...
        for (index, fragment) in fragments.enumerated() {
            ...
            let fragmentPacket = SmartCityChatPacket(
    ...
    private func handleFragment(_ packet: SmartCityChatPacket, from peerID: PeerID) {
        if DispatchQueue.getSpecific(key: messageQueueKey) != nil {
    ...
    private func _handleFragment(_ packet: SmartCityChatPacket, from peerID: PeerID) {
    ...
    private func handleReceivedPacket(_ packet: SmartCityChatPacket, from peerID: PeerID) {
        // Call directly if already on messageQueue, otherwise dispatch
        if DispatchQueue.getSpecific(key: messageQueueKey) == nil {
            messageQueue.async { [weak self] in
    ...
    private func handleAnnounce(_ packet: SmartCityChatPacket, from peerID: PeerID) {
    ...
    private func handleRequestSync(_ packet: SmartCityChatPacket, from peerID: PeerID) {
    ...
    private func handleMessage(_ packet: SmartCityChatPacket, from peerID: PeerID) {
    ...
    private func handleNoiseHandshake(_ packet: SmartCityChatPacket, from peerID: PeerID) {
        ...
        if PeerID(hexData: packet.recipientID) == myPeerID {
            ...
            do {
                if let response = try noiseService.processHandshakeMessage(from: peerID, message: packet.payload) {
                    // Send response
                    let responsePacket = SmartCityChatPacket(
                        type: MessageType.noiseHandshake.rawValue,
                        senderID: myPeerIDData,
                        recipientID: Data(hexString: peerID.id),
                        timestamp: UInt64(Date().timeIntervalSince1970 * 1000),
    ...
    private func handleNoiseEncrypted(_ packet: SmartCityChatPacket, from peerID: PeerID) {
    ...
    private func performCleanup() {
        ...
        collectionsQueue.async(flags: .barrier) { [weak self] in
            ...
            if !self.pendingDirectedRelays.isEmpty {
                var cleaned: [PeerID: [String: (packet: SmartCityChatPacket, enqueuedAt: Date)]] = [:]
