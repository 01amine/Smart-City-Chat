...
/// peer-to-peer communication in SmartCityChat. Acts as the bridge between the transport
...
/// Manages end-to-end encryption for SmartCityChat using the Noise Protocol Framework.
...
final class NoiseEncryptionService {
    ...
    // Thread safety
    private let serviceQueue = DispatchQueue(label: "chat.SmartCityChat.noise.service", attributes: .concurrent)
    ...
    private func canonicalAnnounceBytes(peerID: Data, noiseKey: Data, ed25519Key: Data, nickname: String, timestampMs: UInt64) -> Data {
        var out = Data()
        // context
        let context = "SmartCityChat-announce-v1".data(using: .utf8) ?? Data()
    ...
    /// Sign a SmartCityChatPacket using the noise private key
    func signPacket(_ packet: SmartCityChatPacket) -> SmartCityChatPacket? {
    ...
    /// Verify a SmartCityChatPacket signature using the provided public key
    func verifyPacketSignature(_ packet: SmartCityChatPacket, publicKey: Data) -> Bool {
...
/// These types wrap the underlying SmartCityChat protocol messages with encryption metadata.
