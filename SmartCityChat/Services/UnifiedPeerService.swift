...
final class UnifiedPeerService: ObservableObject, TransportPeerEventsDelegate {
    ...
    @Published private(set) var peers: [SmartCityChatPeer] = []
    @Published private(set) var connectedPeerIDs: Set<PeerID> = []
    @Published private(set) var favorites: [SmartCityChatPeer] = []
    @Published private(set) var mutualFavorites: [SmartCityChatPeer] = []
    ...
    private func updatePeers() {
        ...
        var enrichedPeers: [SmartCityChatPeer] = []
        ...
        enrichedPeers.sort { lhs, rhs in
            ...
            func rank(_ p: SmartCityChatPeer) -> Int { p.isConnected ? 2 : (p.isReachable ? 1 : 0) }
        ...
        var favoritesList: [SmartCityChatPeer] = []
        var mutualsList: [SmartCityChatPeer] = []
        var newIndex: [PeerID: SmartCityChatPeer] = [:]
    ...
    /// Get peer by ID
    func getPeer(by peerID: PeerID) -> SmartCityChatPeer? {
    ...
    var allPeers: [SmartCityChatPeer] { peers }
