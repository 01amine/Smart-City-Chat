...
/// # BinaryProtocol
///
/// Low-level binary encoding and decoding for SmartCityChat protocol messages.
...
/// Implements binary encoding and decoding for SmartCityChat protocol messages.
/// Provides static methods for converting between SmartCityChatPacket objects and
...
struct BinaryProtocol {
    ...
    // Encode SmartCityChatPacket to binary format
    static func encode(_ packet: SmartCityChatPacket, padding: Bool = true) -> Data? {
    ...
    // Decode binary data to SmartCityChatPacket
    static func decode(_ data: Data) -> SmartCityChatPacket? {
    ...
    private static func decodeCore(_ raw: Data) -> SmartCityChatPacket? {
        guard raw.count >= v1HeaderSize + senderIDSize else { return nil }

        return raw.withUnsafeBytes { (buf: UnsafeRawBufferPointer) -> SmartCityChatPacket? in
