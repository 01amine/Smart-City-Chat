...
struct FragmentationTests {
    ...
    func maxSizedFileTransferSurvivesReassembly() async throws {
        ...
        let filePacket = SmartCityChatFilePacket(
            fileName: "limit.bin",
...
extension FragmentationTests {
    ...
    private final class CaptureDelegate: SmartCityChatDelegate, @unchecked Sendable {
        private let lock = NSLock()
        private var _publicMessages: [(peerID: PeerID, nickname: String, content: String)] = []
        private var _receivedMessages: [SmartCityChatMessage] = []
        ...
        var receivedMessages: [SmartCityChatMessage] {
            lock.lock()
            defer { lock.unlock() }
    ...
    private func makeLargePublicPacket(senderShortHex: PeerID, size: Int) -> SmartCityChatPacket {
    ...
    private func fragmentPacket(_ packet: SmartCityChatPacket, fragmentSize: Int, fragmentID: Data? = nil, pad: Bool = true) -> [SmartCityChatPacket] {
        ...
        var packets: [SmartCityChatPacket] = []
