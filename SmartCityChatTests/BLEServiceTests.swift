...
struct BLEServiceTests {
    ...
    @Test func sendPublicMessage() async throws {
        try await confirmation { receivedPublicMessage in
            let delegate = MockSmartCityChatDelegate { message in
    ...
    @Test func simulateIncomingMessage() async throws {
        try await confirmation { receiveMessage in
            ...
            let incomingMessage = SmartCityChatMessage(
                id: "MSG456",
                sender: "RemoteUser",
    ...
    @Test func messageDeliveryHandler() async throws {
        try await confirmation { deliveryHandler in
            service.packetDeliveryHandler = { packet in
                if let msg = SmartCityChatMessage(packet.payload) {
...
private final class MockSmartCityChatDelegate: SmartCityChatDelegate {
    private let messageHandler: (SmartCityChatMessage) -> Void
    
    init(_ handler: @escaping (SmartCityChatMessage) -> Void) {
