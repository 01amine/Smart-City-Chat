...
struct NostrProtocolTests {
    ...
    func testAckRoundTripNIP44V2_Delivered() throws {
        ...
        let embedded = try #require(
            NostrEmbeddedSmartCityChat.encodeAckForNostrNoRecipient(type: .delivered, messageID: messageID, senderPeerID: senderPeerID),
        ...
        // Parse SmartCityChat payload
        #expect(content.hasPrefix("SmartCityChat1:"))
        let base64url = String(content.dropFirst("SmartCityChat1:".count))
        let packetData = try #require(Self.base64URLDecode(base64url))
        let packet = try #require(SmartCityChatPacket.from(packetData), "Failed to decode SmartCityChat packet")
    ...
    @Test func ackRoundTripNIP44V2_ReadReceipt() throws {
        ...
        let embedded = try #require(
            NostrEmbeddedSmartCityChat.encodeAckForNostrNoRecipient(type: .readReceipt, messageID: messageID, senderPeerID: senderPeerID),
